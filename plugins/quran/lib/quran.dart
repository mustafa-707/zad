...
String getSurahName(int surahNumber) {
  if (surahNumber > 114 && surahNumber < 0) {
    throw "No Surah found with given surahNumber $surahNumber";
  }
  return surah[surahNumber]['name'].toString();
}
...
int getVerseCount(int surahNumber) {
  ...
  return int.parse(surah[surahNumber]['aya'].toString());
}
  ...
  for (var i in quranText) {
    if (i['surah_number'] == surahNumber && i['verse_number'] == verseNumber) {
      if (local == 'ar') {
        ...
        verse = i['translation'][local].toString();
      }
      break;
    }
  }
...
Map searchWords(List<String> words) {
  ...
  for (var i in quranText) {
    ...
    for (var word in words) {
      if (i['content'].toString().toLowerCase().contains(word.toString().toLowerCase())) {
        exist = true;
      }
    }
    if (exist) {
